...
- **Plan ID:** `logic-van-hanh-2026-09-16`; **Version:** 2.
- **Status:** `READY` — DEC-1 và DEC-2 đã được người dùng chốt; không còn escalation mở. READY chỉ nói đủ quyết định để bàn giao, không phải lệnh triển khai hay bằng chứng tests đã đạt.
- **History:** v1 giữ nguyên câu hỏi trước khi chốt; v2 ghi quyết định người dùng và contract vùng mới/legacy. Dùng v2 để bàn giao.
...
**DEC-2 — người dùng chốt trong hội thoại ngày 2026-09-16:**

> Vùng riêng thay thế vùng chung; dữ liệu cũ có cả hai loại phải được xử lý tương thích rõ ràng, không âm thầm đổi nghĩa.

Phạm vi: frontend mới dùng replacement tường minh; payload/artifact legacy vẫn additive khi được chấp nhận hợp lệ. JSON cũ theo index không chứng minh được bộ cue phải yêu cầu chọn lại, không tự migrate sang replacement. Exact contract tại LD-7.
...
- Giữ URL endpoint/CLI hiện tại; mở rộng field mode tương thích ngược đúng LD-7 và nội bộ cần thiết để phân biệt lượt chạy mới với tiếp tục lượt đang chờ.
...
| AC-2 | Nghĩa vùng chung/riêng nhất quán giữa UI, HTTP, artifact và render, theo DEC-2/LD-7; không âm thầm diễn giải lại dữ liệu cũ | V-02, V-08, V-10 |
...
| CS-6 | Nghĩa common/private và tương thích payload/JSON cũ phải được xác định trước đổi | DEC-2; UI dat_pham_vi; legacy tests; discriminator LD-7 |
...
- Layout nội bộ: `work/tai_len/nguon/nguon-<sha256-scope>-<sha256-video>.media`; output `work/tai_len/<cid>/<stem>_vi.mp4`. Hai digest dùng đủ 64 ký tự hex; scope digest là `chu_ky(["web-cache-v1", nhom_ten_or_none])`. Work vẫn qua `thu_muc_lam_viec(canonical_source)` như contract cũ. Stem mang cả hai digest để không thu toàn bộ identity mới về 8 ký tự hash-path; cấu trúc phẳng tránh nhân đôi digest trong đường dẫn Windows. Đuôi nội bộ cố định để đổi extension/tên upload không đổi identity; validation vẫn dựa filename upload và ffprobe nội dung.
...
- Chữ ký vùng theo cue bao gồm SHA256 SRT gốc hợp lệ/đã normalize, bên cạnh hash video, W/H, version và mode vùng theo LD-7.
...
#### LD-7 — contract replacement mới và additive legacy (đã chốt)

- Protects: AC-2/6/8, CS-6/7, IC-5/6; source/authority DEC-2. Discriminator tường minh bảo toàn semantics payload cũ và thực hiện behavior mới, không đổi nghĩa ngầm của cue:null.
- Loại: đổi `_cue_cua(cue=None)` cho mọi input; rewrite JSON cũ hàng loạt; bỏ test additive legacy đúng để chỉ còn test replacement.

**LD-7a. Wire contract, mặc định và validation**

POST `/api/cong-viec/{cid}/hop` giữ URL/fields/vung shape hiện tại, thêm field top-level:
...
```json
{
  ...
  "vung": [
    {"x": 0.2, "y": 0.8, "w": 0.6, "h": 0.1, "cue": null},
    {"x": 0.2, "y": 0.1, "w": 0.6, "h": 0.1, "cue": [1]}
  ],
  "luu_nhom": false
}
```

- Allowed strings đúng hai giá trị `cong_them`, `thay_the`, phân biệt case. Field **vắng mặt** = cong_them. Field có mặt nhưng null/bool/số/chuỗi khác = HTTP 400 trước thay đổi job/nhóm/artifact.
- Frontend mới luôn gửi che_do_vung=thay_the khi gửi vùng. Bỏ qua blur giữ co_blur=false, không lưu đè box nhóm; nếu mode có mặt vẫn validate dù co_blur=false.
- Response thành công giữ id/trang_thai/vung, thêm che_do_vung đã nhận. Không thêm CLI flag hay upload form mode chỉ để chọn một box chung.
- TuyChon thêm field nội bộ che_do_vung mặc định cong_them; dat_hop nhận mode đã validate. Không chèn tham số positional vào giữa làm lệch callers cũ.
- Một box dict cũ/list legacy vẫn additive. Shared validator markbox kiểm mode/constraints; API chỉ map lỗi thành 400, không viết validator thứ hai.
- thay_the: tối đa một common (cue:null/missing); một index không thuộc hai private regions khác nhau, kể cả hai box cùng tọa độ. Duplicate trong **cùng** private region vẫn deduplicate như hiện tại. Nhiều common/cross-region duplicate → 400.
- cong_them: giữ khả năng nhiều vùng cùng active; không áp uniqueness của replacement lên legacy.
- No common: chỉ cue gán private có blur. Common only: như box chung hiện tại. Mọi cue private: common không blur nhưng có thể vẫn là primary theo LD-7c. Danh sách vùng rỗng vẫn invalid; bỏ blur dùng co_blur=false.

**LD-7b. Artifact và validity**

- Artifact mới: `{"co_blur":true,"che_do_vung":"thay_the|cong_them","vung":[...]}`. Giá trị mode thật là một trong hai string, không phải string chứa dấu |. `VER['vung_blur']=3` (base=2), signature bao gồm video/geometry/sub-source-hash/mode. Audio/sub/dịch versions và PROMPT_VER giữ nguyên.
- Cache hợp lệ mang mode nào thì reuse mode đó; không dùng default TuyChon của upload lại để đổi mode đã lưu.
- File/payload thiếu mode nghĩa legacy cong_them; **nghĩa đọc được không đủ chứng minh cache còn hợp lệ**. Artifact legacy có private cue nhưng thiếu source fingerprint → chọn lại, giữ nguyên file đến khi có lựa chọn mới hợp lệ.
- Box legacy chung độc lập index chỉ reuse khi metadata cũ chứng minh đúng video/geometry và validator pass. Metadata mất/hỏng/unknown mode không được default im lặng thành cache hợp lệ.
- Không quét/migration/xóa layout cũ. Khi có lựa chọn mới, atomic-replace artifact của đúng work đang xử lý. API legacy mới gửi vùng vẫn chạy additive, metadata mới ghi mode explicit cho lần sau.
- Khi cần chọn lại vì legacy/stale source, không tự fallback group box để che mất private cũ. Frame/cue phải từ nguồn hiện tại đã xác minh.

**LD-7c. Cue, thời gian và primary box**

1. Giữ raw regions đã validate cho lưu trữ/hop_chinh. Replacement: P là hợp index private; common nhận cues không thuộc P, private nhận cues của nó.
2. Tính khoảng từng region bằng render.khoang_mo hiện có (padding/gap/TOI_DA giữ nguyên).
3. Padding/merge có thể bắc cầu qua private. Bổ sung **time-mask precedence**: common active khi mask_common đúng **và không private mask nào active**. Private thắng cả biên inclusive; không epsilon đoán FPS.
4. Union private masks chỉ gộp overlap/touch, không nới gap mới hoặc truncate để ép số khoảng. Không bỏ exclusion khi hơn 50 khoảng/nhiều private regions.
5. render.ket_xuat thêm keyword-only loai_tru_chung=None (tên Việt tương đương được ghi trong handoff) nhận exclusions. Default None giữ calls cũ. Exclusion chỉ áp common, không triệt private lẫn nhau hoặc làm additive đổi kết quả; biểu thức ffmpeg chỉ từ số validate, không nội suy user text.
6. Chọn primary bằng markbox.hop_chinh từ **raw** regions trước khi trừ cue/filter bỏ khoảng rỗng. Điều phối truyền style['hop_chinh'] nội bộ; field vắng thì renderer giữ behavior cũ. Group default dùng cùng raw primary. Không chọn lại theo effective cue count.
7. Không có common: giữ private phủ nhiều cue nhất; tie giữ thứ tự hiện tại. Không thêm UI chọn vị trí chữ.

Vẫn một lần encode. TOI_DA của từng region giữ nguyên; exclusion là điều kiện ưu tiên bổ sung, không gộp lại làm common xuất hiện trên private để rút ngắn filtergraph. Lỗi ffmpeg phải báo rõ/giữ output tốt cũ; runtime fixture kiểm cả exclusion nhiều khoảng.
...
#### ES-2 — RESOLVED: replacement mới, legacy giữ nghĩa rõ ràng

- Authority/source: câu trả lời trực tiếp của người dùng tại DEC-2.
- Decision: frontend mới dùng replacement; discriminator/default/persistence/time precedence theo LD-7; raw legacy giữ additive, không reinterpret.
- Blocks đã gỡ: STEP-4/5, AC-2, V-02/08/10. **Unresolved Escalations: None.**
- Nếu gặp consequential ambiguity mới, tạo escalation mới; không mở lại DEC-1/2 chỉ vì thiếu giấy tờ.

### 11. Implementation Plan — chỉ nhiệm vụ mã sản phẩm của Claude

**Thứ tự:** STEP-1 → STEP-2 → STEP-3 → STEP-6; STEP-4 theo DEC-2/LD-7; STEP-5 sau STEP-3/4/6; STEP-7 sau các bước mã. Luna chuẩn bị/chạy các V ở Phần 2, không chèn test/review checklist vào phần executor.
...
#### STEP-4 — replacement có mode tường minh, bảo toàn additive legacy

- Depends on: DEC-2/LD-7, STEP-1/2; schema/mode đã khóa trong v2.
- Targets: `web/app.js:dat_pham_vi`, `ve_khung`, `xay_vung`, `gui_hop`; `api/app.py:nhan_hop`; `api/viec.py:dat_hop`; `pipeline/markbox.py:kiem_vung`, `hop_chinh`; `pipeline/dieu_phoi.py:TuyChon`, `_cue_cua`, `_buoc_hop`, `_hop_chung`; `pipeline/render.py:ket_xuat`.
- Required change theo thứ tự:
  1. Shared validation nhận mode; kiểm allowed values, replacement uniqueness/common count và index range; legacy rules giữ nguyên.
  2. Bổ sung mode ở TuyChon/job, POST vùng và artifact; giữ default legacy cho callers cũ. Mode của artifact hợp lệ là authoritative khi không có box mới.
  3. Frontend gui_hop gửi thay_the; xay_vung vẫn gửi raw common/private để giữ provenance/primary. Preview private thay common như mô tả hiện tại.
  4. Resolve common cue complement, private time masks và union exclusion theo LD-7c tại pipeline; API không tự tính khoảng.
  5. Renderer nhận common exclusion keyword-only và raw primary qua style; common enable phải có phủ định private-mask để gap merge không tạo lại double blur; không epsilon đoán theo FPS.
  6. Group default/style dùng cùng raw primary; common empty sau resolve không làm primary chuyển vùng khác.
- Preserved: AC-2/8; CS-6/7/10; IC-5/6; LD-7. Private có padding/merge như trước, nhưng replacement luôn thắng common ở nơi private mask active.
- Freedom: mechanics DOM/helper/ffmpeg expression đáp ứng LD-7; không redesign UI hoặc đổi mode/default.
- Stop: cần wire/schema/precedence khác LD-7 hoặc phải migration dữ liệu cũ.
- Handoff verification: V-02/08/10/11. Expected: replacement không double blur, legacy additive như cũ, primary không drift.

#### STEP-5 — invalidate vùng theo cue và chống snapshot cũ

- Depends on: STEP-3/4/6; mode/persistence đã chốt LD-7.
...
- Freedom: MD-1; mode compatibility theo LD-7.
...
- Mô tả rõ identity mới, phạm vi cache mới, không migration cũ, output riêng, vùng theo LD-7, restart/stale lock, phân biệt force mới/resume.
...
#### V-02 — semantics và compatibility vùng (`BOTH`)
...
- thay_the: tại 10.5s dưới không active, trên active; tại 0.5s chỉ dưới active. Cùng raw payload thiếu mode hoặc cong_them: tại 10.5s cả hai active; legacy regression bắt buộc.
- Kiểm thêm không private, không common, mọi cue private, private trùng index, common sau resolve không còn cue, index ngoài range, bool/NaN/infinity trong box. Theo LD-7: thay_the từ chối nhiều common/cross-region duplicate; cong_them chấp nhận; explicit null/unknown mode=400, field vắng=cong_them.
- Kiểm primary style/default nhóm bằng box có kích thước khác rõ ràng; chúng phải cùng dùng hộp đại diện theo CS-7.
- Golden legacy giữ additive nếu validity chứng minh được; private artifact thiếu source hash yêu cầu chọn lại, không auto-convert. Mode cached thắng default của upload lại.
- Regression padding/gap bắc cầu qua private: time-mask vẫn tắt common trên private, kể cả biên inclusive. Test >50 exclusion intervals không truncate/gộp lấp private.
- Giữ assertion cộng vùng trong test legacy; thêm test thay_the. Frontend giữ common.cue=null nhưng boxPayload.che_do_vung phải bằng thay_the.
- **Covers:** AC-2/8, CS-6/7, IC-5/6, LD-4/7, DEC-2.
...
- Single box không phụ thuộc index và JSON legacy thiếu fingerprint theo LD-4/7; fixture bao gồm group default để không che mất yêu cầu chọn lại vùng riêng.
...
- Thêm case vùng riêng theo LD-7, mode/payload compatibility, stale 409 hiện thông báo, restart `loi` dừng polling và mở upload lại.
...
- Giữ test additive legacy và thêm replacement; khi đổi assertion HTTP payload mới ghi old→new/LD-7, không xóa case legacy hay nới valid tests.
...
- Giữ 720p/1080p, có audio, duration lệch ≤0.1s, không blur/có blur/hai vùng; thêm fixture common/private theo DEC-2/LD-7 chạy qua điều phối thật đến render thật, không chỉ truyền hai vùng đã lọc sẵn.
...
**Covers:** AC-1…9 tương ứng contract LD-7, CS-1…11, LD-1…7, MD-1/2/3.
...
| G-5 Test | V-01…09 theo LD-7 đã chốt, full offline + browser | Red→green có lý do, mọi test mới thực sự chạy | Finding+reproduction; chưa qua Runtime acceptance |
...
| F-2 Vùng | STEP-4 Claude, theo LD-7 | Luna | V-02 compatibility; V-08 UI; V-10 ảnh render |
...
**Thực thi theo slices:** F-1/F-6 trước; F-3/F-4 phối hợp interface checkpoint; sau đó F-2/F-5 và runtime toàn chuỗi. Không còn quyết định mở nhưng chưa gọi sáu lỗi đã sửa trước khi mọi AC có evidence mới.
...
Quyết định đã dùng: DEC-1, DEC-2/LD-7; deviations nếu có.
...
- V2 đã giải quyết DEC-1/2 và có exact contract LD-7. READY không phải bằng chứng implementation/tests hoàn tất. Khi có quyết định material mới, ghi v3/decision record trước mutation phụ thuộc, không sửa lén lịch sử.
